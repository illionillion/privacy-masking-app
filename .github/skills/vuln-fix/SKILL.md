...
```
1. pnpm i          … 依存関係をインストール（既存 lockfile に従う。依存変更後は lockfile が更新され得る）
2. pnpm audit      … 脆弱性を検出
3. トリアージ         … 対応 / 無視 を判断
4. 修正を適用         … バージョンアップ or overrides
5. pnpm i + audit  … 再インストール後に audit で解消を確認
...
| `Patched in` | 修正が取り込まれたバージョンの目安。`No patch available` は公式パッチ未提供を示す。無視・対応・相談の判断は **Step 3** で行う |
