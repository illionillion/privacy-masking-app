...
name: github-ops
description: >-
  Issue作成・PR作成・コミット・gh CLI 操作の手順を案内する。Issue・PR・コミット・プッシュ・インラインレビュー返信・ラベル確認・本文ファイル・Conventional Commits を含む。「Issue作って」「PR立てて」「コミットして」「レビューコメント確認」などの依頼時に使う。
...
2. **本文ファイル生成**  
   ...
   テンプレートは `.github/pull_request_template.md` に従い全項目を埋める。  
...
push する前に、PR本文（`.github/DRAFTS/<ブランチ名>-pr.md`）が実装内容と乖離していないか確認する。  
以下のいずれかに該当する場合は、push 前に PR 本文を更新してから `gh pr edit` で反映する：
...
GitHub Copilot 自動コードレビューは **push ごとに走る**。不要なレビュー連発を避けるため、push は意図した単位にまとめる。

- **ローカルコミット**: 論理単位で複数コミットしてよい（1コミット1目的は維持）
- **リモートへの push**: 作業単位が揃ってから **まとめて1回** push する
- **push 前に実施**: 関連テストの実行、PR本文の更新（上記「PR本文の追従」）、`git status` で意図しない変更がないことの確認
- **避けること**: コミットのたびに push、細切れの連続 push、レビュー対応を1件直すたびに push
- **レビュー対応後**: 未対応指摘を一通り修正・返信してから **1回 push** する
- **例外**: ユーザーが明示的に push を依頼した場合、または PR 初回公開（最初の push）時
